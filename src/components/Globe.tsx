...
const GlobeComponent = forwardRef<GlobeComponentRef, GlobeProps>(
    ({ width = 600, focusTime = 2500, markers = [], darkMode = false }, ref) => {
        ...
        useEffect(() => {
            ...
            const globe = createGlobe(globeRef.current, {
                ...
                onRender: (state) => {
                    ...
                    const deltaPhi = shortestAngularDistance(normalizeAngle(currentPhi), focusPhi);
                    currentPhi += deltaPhi * 0.05;
                    currentTheta += (focusTheta - currentTheta) * 0.05;
                    state.phi = currentPhi;
                    state.theta = Math.max(-Math.PI / 2, Math.min(Math.PI / 2, currentTheta));
